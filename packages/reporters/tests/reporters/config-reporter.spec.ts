...
describe('TextConfigReporter', () => {
  ...
  describe('renderHealthReport()', () => {
    ...
    it('left-aligns issue rows and aligns suggestions under the message', () => {
      ...
      const issueLine = physicalLines.find((line) =>
        line.includes('Output directory does not exist')
      )!;
      const suggestionLine = physicalLines.find((line) =>
        line.includes('Create the directory')
      )!;
      ...
      expect(suggestionLine.trimStart()).toContain(
        'Create the directory or update outputPath.'
      );
    });
  });
});
