...
import { createTestOutput } from '../../src/formatting/output.js';
...
describe('SarifReporter', () => {
  ...
  describe('report()', () => {
    it('emits a SARIF 2.1.0 document for zero results', () => {
      reporter.report([]);
      reporter.summary(
        summary({ totalErrors: 0, totalFiles: 0, totalTasks: 0 })
      );
    ...
    it('maps rule failures to SARIF results and rule metadata', () => {
      reporter.report([
        makeResult('src/app.component.ts', { ruleName: 'prefer-on-push' }),
      ]);
    ...
    it('maps warn severity to SARIF warning level', () => {
      reporter.report([
        makeResult('src/app.component.ts', { severity: 'warn' }),
      ]);
    ...
    it('sorts results by file path then source position', () => {
      ...
      expect(
        parsed.runs[0].results.map((result: any) => result.message.text)
      ).toEqual(['a', 'b', 'z']);
    });

    it('includes parse errors as tool execution notifications', () => {
      reporter.parseErrors([
        { filePath: 'src/broken.ts', message: 'Unexpected token' },
      ]);
      reporter.report([]);
      ...
      expect(
        parsed.runs[0].invocations[0].toolExecutionNotifications[0]
      ).toMatchObject({
        level: 'warning',
        message: { text: 'Unexpected token' },
      });
      expect(
        parsed.runs[0].invocations[0].toolExecutionNotifications[0].locations[0]
