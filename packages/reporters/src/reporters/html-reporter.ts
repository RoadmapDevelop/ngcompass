...
const CATEGORY_RULES: ReadonlyArray<{
...
}> = [
  {
    category: 'Security',
    match: (n) =>
      n === 'no-bypass-sanitization' || n === 'template-no-unsafe-bindings',
  },
  {
    category: 'Template',
    match: (n) =>
      n === 'template-prefer-control-flow' ||
      n === 'template-no-async-pipe-duplication',
  },
  {
  ...
  },
  {
    category: 'Modern API',
    match: (n) =>
      n.startsWith('signal-prefer-') ||
      n === 'prefer-inject-over-constructor-di',
  },
  {
    category: 'SSR',
    match: (n) =>
      n === 'no-document-access' ||
      n === 'prefer-after-render-over-after-view-init',
  },
  {
    category: 'Testing',
...
function categoryColorClass(category: string): string {
  switch (category) {
    ...
    case 'Reactivity':
      return 'cat-reactivity';
    case 'Modern API':
      return 'cat-modern-api';
    case 'Security':
      return 'cat-security';
    case 'SSR':
      return 'cat-ssr';
...
function bucketFiles(results: ReadonlyArray<RuleResult>): FileBucket[] {
  ...
  return [...grouped.entries()]
    .map(([filePath, failures]) => {
      ...
      const errorCount = sortedFailures.filter((failure) =>
        isErrorSeverity(failure.severity)
      ).length;
    ...
    .sort((a, b) => {
      ...
      if (b.failures.length !== a.failures.length)
...
function buildCategoryChart(
  categoryCounts: ReadonlyArray<[string, number]>,
  total: number
): string {
...
function buildIssueRow(failure: RuleFailure): string {
  const severityClass = isErrorSeverity(failure.severity)
    ? 'is-error'
    : 'is-warning';
...
function buildFileCard(bucket: FileBucket, index: number): string {
  ...
  const issues = bucket.failures
    .map((failure) => buildIssueRow(failure))
    .join('\n');
...
function buildHtml(
  ...
  const totalFiles =
    summary.discoveredFiles ?? summary.scannedFiles ?? summary.totalFiles;
  ...
  const statusClass =
    status.status === 'failed'
      ...
      : status.status === 'passed-with-warnings'
        ...
        : 'pass';
  const cachedCopy =
    typeof summary.cachedTasks === 'number' && summary.cachedTasks > 0
...
<section class="card parse-errors">
  ...
  const filesHtml =
    fileBuckets.length > 0
      ? fileBuckets
          .map((bucket, index) => buildFileCard(bucket, index))
          .join('\n')
      : `
<div class="empty">
  <div>
