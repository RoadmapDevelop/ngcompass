...
function toSarifResult(failure: RuleFailure): SarifResult {
  return {
    ...
    locations: [toLocation(failure.filePath, failure.line, failure.column)],
  };
}
...
function toParseErrorNotification(error: ParseError): SarifNotification {
  return {
    ...
    locations: [toLocation(error.filePath, 1, 1)],
  };
}
...
export class SarifReporter implements Reporter {
  ...
  summary(stats: ResultSummary): void {
    this.out.write(
      JSON.stringify(
        buildSarifReport(this.resultBuffer, this.parseErrorBuffer, stats),
        null,
        2
      )
    );
