...
const logScanStart = (options: ScanOptions): void => {
...
const logFiltering = (fileCount: number, filteredCount: number): void => {
  debug(
    'scanner',
...
const logScanEnd = (stats: ScanStatistics, scanTime: number): void => {
  debug(
    'scanner',
...
const logAndReturnError = (phase: string, error: Error): void => {
...
const groupCachedFilesByExtension = (
  files: ReadonlyArray<string>
...
const normalizeCachedStats = (value: unknown): ScanStatistics | null => {
...
const serializeScanStats = (stats: ScanStatistics): CachedScanStatistics => ({
  totalFiles: stats.totalFiles,
...
const loadTsConfigPatterns = async (
  tsConfigPath: string,
  rootDir: string
  ...
  try {
    ...
    debug(
      'scanner',
      `Failed to load tsconfig patterns from ${tsConfigPath}: ${describeError(err)}`
    );
    return null;
  }
};

const mergeTsConfigPatterns = (
  patterns: ExpandedPatterns,
