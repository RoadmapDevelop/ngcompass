/**
 * Unit / Integration Tests — glob.ts
 *
 * Uses real temporary directories on disk so the glob engine behaves exactly
 * as it would in production.  Tests cover: executeGlob, patternsLikelyHaveMatches.
 */
...
describe('executeGlob', () => {
    ...
    it('returns absolute paths for every discovered file', async () => {
        ...
        for (const file of result.data.files) {
            expect(file).toMatch(/^([A-Z]:[\\/]|\/)/, `Expected absolute path, got: ${file}`);
        }
    });
...
describe('patternsLikelyHaveMatches', () => {
    ...
    it('returns false when every include pattern is an exclusion (starts with !)', () => {
        expect(
            patternsLikelyHaveMatches({ include: ['!**/*.ts', '!**/*.html'], ignore: [] }),
        ).toBe(false);
    });
    ...
    it('returns true when mixed negation and positive include patterns are present', () => {
        expect(
            patternsLikelyHaveMatches({ include: ['**/*.ts', '!**/*.spec.ts'], ignore: [] }),
        ).toBe(true);
    });
