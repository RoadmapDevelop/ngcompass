...
import { ComponentDependencyGraph } from '../../src/resource-discovery/component-graph.js';
  ...
  describe('decorator fallback', () => {
    it('resolves templateUrl when convention fails', async () => {
      await withTmpDir(async (dir) => {
        ...
        await fs.writeFile(
          comp,
          `@Component({ templateUrl: './templates/app.html' })`
        );
    ...
    it('resolves styleUrls when convention finds no styles', async () => {
      await withTmpDir(async (dir) => {
        ...
        await fs.writeFile(
          comp,
          `@Component({ styleUrls: ['./styles/app.scss'] })`
        );
    ...
    it('resolves Angular 17+ styleUrl (singular) when convention finds no styles', async () => {
      await withTmpDir(async (dir) => {
        ...
        await fs.writeFile(
          comp,
          `@Component({ styleUrl: './app.styles.scss' })`
        );
        await fs.writeFile(style, '');
