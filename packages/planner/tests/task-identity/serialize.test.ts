...
import { serializePlan, deserializePlan } from '../../src/task-identity/serialize.js';
import type {
  ExecutionPlanOutput,
  Task,
  RuleTask,
...
const makeOutput = (
  tasks: Task[],
  units: Record<string, FileAnalysisUnit>
): ExecutionPlanOutput => ({
  tasks,
  plan: units,
  indexes: {
...
describe('serializePlan / deserializePlan (round-trip)', () => {
  it('round-trips a basic plan with one rule', () => {
    ...
    expect(restoredUnit.tasks[0].inputs.typescript.path).toBe(
      '/src/app.component.ts'
    );
    expect(restoredUnit.tasks[0].inputs.template?.path).toBe(
  ...
  it('round-trips needsTypeChecker flag correctly', () => {
    ...
    expect(
      restored.plan['/src/app.component.ts'].tasks[0].needsTypeChecker
    ).toBe(true);
  });
  ...
  it('round-trips spec input', () => {
    const ruleTask = makeRuleTask('spec-rule', {
      inputs: {
        ...
        spec: {
          path: '/src/app.component.spec.ts',
          hash: 'spec-hash',
          needsAst: true,
        },
      },
    ...
    expect(
      restored.plan['/src/app.component.ts'].tasks[0].inputs.spec?.path
