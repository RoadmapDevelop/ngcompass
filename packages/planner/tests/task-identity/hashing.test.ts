...
import {
  ...
  calculateGlobalHash,
  warmupHashCache,
} from '../../src/task-identity/hashing.js';
...
describe('calculateFileHash', () => {
  ...
  it('produces a different hash when the template hash changes', () => {
    ...
    const rules = [makeRule('rule-a')];
    expect(calculateFileHash(inputs1, rules)).not.toBe(
      calculateFileHash(inputs2, rules)
    );
  });
...
describe('calculateTaskId', () => {
  ...
  it('incorporates cacheKeyCtx when provided', () => {
    ...
    const withDifferentCtx = calculateTaskId(
      'rule-a',
      inputs,
      {},
      {
  ...
  it('hydrates the in-memory hash cache from metadata when size and mtime match', async () => {
    ...
    try {
      ...
      >([
        [
          filePath,
          { mtime: stats.mtimeMs, size: stats.size, hash: 'cached-hash' },
        ],
      ]);
      const metaCache = {
        ...
        set: vi.fn(
          async (
            key: string,
            value: { mtime: number; size: number; hash: string }
          ) => {
            meta.set(key, value);
          }
        ),
