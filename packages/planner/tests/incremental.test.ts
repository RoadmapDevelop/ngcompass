...
/** Creates a minimal ResultCache mock. */
const makeCache = (cachedIds: string[] = []): ResultCache => {
    const cached = new Set(cachedIds);
    return {
        hasMany: vi.fn(async (ids: string[]) => new Set(ids.filter((id) => cached.has(id)))),
        ...
        getManyWithMetadata: vi.fn(async (ids: string[]) => {
            ...
            for (const id of ids) {
                if (cached.has(id)) {
                    map.set(id, { metadata: { taskId: id, timestamp: Date.now() - 1000, hits: 5 } });
                }
            }
            return map;
        }),
...
describe('filterCachedTasks', () => {
    ...
    it('loads cached results when loadCachedResults is true', async () => {
        ...
        const result = await filterCachedTasks(tasks, cache, { loadCachedResults: true });
...
describe('pruneStaleCache', () => {
    ...
    it('prunes entries that exceed maxAge', async () => {
        ...
        const veryOldTimestamp = Date.now() - 10 * 24 * 60 * 60 * 1000; // 10 days ago

        const cache = {
            getManyWithMetadata: vi.fn(async () =>
                new Map([['old-t1', { metadata: { taskId: 'old-t1', timestamp: veryOldTimestamp, hits: 5 } }]])
            ),
            delete: vi.fn(async () => {}),
        } as unknown as ResultCache;
    ...
    it('prunes entries below minHits', async () => {
        ...
        const cache = {
            getManyWithMetadata: vi.fn(async () =>
                new Map([['cold-t1', { metadata: { taskId: 'cold-t1', timestamp: Date.now(), hits: 0 } }]])
            ),
            delete: vi.fn(async () => {}),
        } as unknown as ResultCache;
    ...
    it('does not prune hot recent entries', async () => {
        ...
        const cache = {
            getManyWithMetadata: vi.fn(async () =>
                new Map([['hot-t1', { metadata: { taskId: 'hot-t1', timestamp: Date.now(), hits: 10 } }]])
            ),
            delete: vi.fn(async () => {}),
        } as unknown as ResultCache;
