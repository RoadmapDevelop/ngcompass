...
const KNOWN_SAFE_PIPES =
  /\|\s*(safeHtml|safeStyle|safeUrl|safeResourceUrl|sanitize|sanitizeHtml|sanitizeUrl|trustHtml|trustStyle|trustUrl|trustResourceUrl|bypassSecurity)\b/i;
const SANITIZE_METHOD_CALL =
  /\b(getSafeHtml|getSafeUrl|getSafeStyle|getSafeResourceUrl|sanitize|sanitizeHtml|sanitizeUrl|trustHtml|bypassSecurity)\s*\(/i;
...
export const templateNoUnsafeBindingsRule = createTemplateAttributeRule(
  ...
  (node: TemplateAttributeNode, context: RuleContext): RuleFailure | null => {
    ...
    const { line, column } = context.locator.location(
      getTemplateAbsoluteOffset(context, node.sourceSpan.start)
    );
    return {
