...
describe('template-no-array-literal-binding', () => {
    it('has correct name and streamType', () => {
        expect(templateNoArrayLiteralBindingRule.name).toBe('template-no-array-literal-binding');
...
describe('template-no-object-literal-binding', () => {
    it('has correct name and streamType', () => {
        expect(templateNoObjectLiteralBindingRule.name).toBe('template-no-object-literal-binding');
...
describe('template-trackby-required', () => {
    ...
    it('does NOT flag @for block with track expression', () => {
        const ctx = makeContext('', '/src/app.component.ts');
        // Angular splits @for parameters on semicolons, so `track item.id` is a separate
        // parameter entry starting with `track`.
        const fakeAnalysis = {
            attributes: [],
            blocks: [
                {
