/**
 * rule-utils — unit tests
 *
 * Directly tests every exported utility function in rule-utils.ts.
 * Rules exercise these helpers indirectly; this suite targets them in isolation
 * so regressions surface with precise failure messages.
 *
 * Covered exports:
 *  unwrapNode · isMemberExpressionLike · getStaticPropertyName · getNodeStart
 *  isCalleeNamed · getCalleeName · isSubscribeCall
 *  getObjectProperty · isLiteralTrue · isLiteralNullOrUndefined
 *  childNodes · isConstructorMethod · isMethodDefinition
 *  getConstructorMember · getMethodBody · getMethodName · getClassBody
 *  isAllowedEffectCall · findEffectCalls · getCallbackArg · getFunctionBody
 *  collectRxjsAliases · collectAllRxjsAliases
 *  hasTeardownInPipeCall · hasTeardownInReceiverChain · getOperatorNameFromPipeArg
 *  getTemplateAbsoluteOffset
 *  findObservableSourceCall · isLikelyHttpObservable
 *  getParamsArray · getParamIdentifierName · getParamTypeName
...
function memberExpr(object: AstNode, property: AstNode, computed = false): AstNode {
...
describe('unwrapNode', () => {
    ...
    it('unwraps ParenthesizedExpression', () => {
        const inner = identifier('x');
        expect(unwrapNode(n('ParenthesizedExpression', { expression: inner }))).toBe(inner);
    });
...
describe('isMemberExpressionLike', () => {
    it.each(['MemberExpression', 'StaticMemberExpression', 'OptionalMemberExpression'])(
        'returns true for %s',
        (type) => expect(isMemberExpressionLike(n(type))).toBe(true),
    );
...
describe('getStaticPropertyName', () => {
    ...
    it('returns the string value for a computed string literal', () => {
        const expr = n('MemberExpression', { property: literal('key'), computed: true });
        expect(getStaticPropertyName(expr)).toBe('key');
    });

    it('returns empty string for a computed non-literal property', () => {
        const expr = n('MemberExpression', { property: identifier('dynamic'), computed: true });
...
// ---------------------------------------------------------------------------
// isLiteralTrue / isLiteralNullOrUndefined
// ---------------------------------------------------------------------------
...
describe('childNodes', () => {
    ...
    it('does not yield skip-keys (parent, span, loc, start, end, type)', () => {
        ...
        // span is skipped; body is yielded
...
describe('isConstructorMethod', () => {
    it('returns true when kind === "constructor"', () => {
        expect(isConstructorMethod(n('MethodDefinition', { kind: 'constructor' }))).toBe(true);
    });

    it('returns true when key is Identifier named "constructor"', () => {
        expect(isConstructorMethod(n('MethodDefinition', { key: identifier('constructor') }))).toBe(true);
    });

    it('returns false for a regular method', () => {
        expect(isConstructorMethod(n('MethodDefinition', { key: identifier('ngOnInit') }))).toBe(false);
    });
...
describe('getConstructorMember', () => {
    it('returns the constructor method from a class body', () => {
        const ctor = n('MethodDefinition', { kind: 'constructor', key: identifier('constructor') });
...
describe('getMethodBody', () => {
    it('returns the body of a method definition', () => {
        ...
        const def = n('MethodDefinition', { value: n('FunctionExpression', { body }) });
...
describe('getMethodName', () => {
    it('returns the name for an Identifier key', () => {
        expect(getMethodName(n('MethodDefinition', { key: identifier('myMethod') }))).toBe('myMethod');
    });

    it('returns the string literal value for a Literal key', () => {
        expect(getMethodName(n('MethodDefinition', { key: literal('literalMethod') }))).toBe('literalMethod');
    });
...
describe('isAllowedEffectCall', () => {
    it('returns true when options.injector is set to a non-null value', () => {
        const options = n('ObjectExpression', {
            properties: [n('Property', { key: identifier('injector'), value: identifier('destroyRef') })],
        });
        const call = callExpr(identifier('effect'), [n('ArrowFunctionExpression'), options]);
        expect(isAllowedEffectCall(call)).toBe(true);
    });

    it('returns true when options.manualCleanup is true', () => {
        const options = n('ObjectExpression', {
            properties: [n('Property', { key: identifier('manualCleanup'), value: literal(true) })],
        });
        const call = callExpr(identifier('effect'), [n('ArrowFunctionExpression'), options]);
        expect(isAllowedEffectCall(call)).toBe(true);
    });
...
describe('findEffectCalls', () => {
    it('returns an effect() call that is not allowed', () => {
        const effectCall = callExpr(identifier('effect'), [n('ArrowFunctionExpression')]);
        const program = n('Program', { body: [n('ExpressionStatement', { expression: effectCall })] });
    ...
    it('does not return an allowed effect() call', () => {
        const options = n('ObjectExpression', {
            properties: [n('Property', { key: identifier('injector'), value: identifier('ref') })],
        });
        const effectCall = callExpr(identifier('effect'), [n('ArrowFunctionExpression'), options]);
        const program = n('Program', { body: [n('ExpressionStatement', { expression: effectCall })] });
...
describe('getOperatorNameFromPipeArg', () => {
    it('returns identifier name for a bare Identifier arg', () => {
        expect(getOperatorNameFromPipeArg(identifier('takeUntilDestroyed'))).toBe('takeUntilDestroyed');
    });
...
describe('findObservableSourceCall', () => {
    ...
    it('walks through pipe() to the underlying source call', () => {
        ...
        const piped = callExpr(
            memberExpr(sourceCall, identifier('pipe')),
            [callExpr(identifier('map'), [n('ArrowFunctionExpression')])],
        );
...
describe('isLikelyHttpObservable', () => {
    ...
    it('returns false for a plain identifier call', () => {
        expect(isLikelyHttpObservable(callExpr(identifier('doSomething')))).toBe(false);
    });
...
describe('getParamIdentifierName', () => {
    ...
    it('returns the name from an AssignmentPattern left side', () => {
        const param = n('AssignmentPattern', { left: identifier('x'), right: literal(0) });
...
describe('getParamTypeName', () => {
    it('returns the type name from a TSTypeReference', () => {
        const typeNode = n('TSTypeReference', { typeName: identifier('MyService') });
        const param = n('Identifier', { name: 'svc', typeAnnotation: { typeAnnotation: typeNode } as any });
    ...
    it('unwraps TSParameterProperty before reading the annotation', () => {
        ...
        const inner = n('Identifier', { name: 'router', typeAnnotation: { typeAnnotation: typeNode } as any });
