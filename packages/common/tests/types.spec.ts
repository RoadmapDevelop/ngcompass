...
describe('RuleCategory', () => {
  ...
  it('has no unexpected keys', () => {
    const actualKeys = Object.keys(RuleCategory).filter((k) =>
      isNaN(Number(k))
    );
...
describe('ConfigurationError', () => {
  ...
  it('sets name to "ConfigurationError"', () => {
    expect(new ConfigurationError('bad config').name).toBe(
      'ConfigurationError'
    );
  });
});
...
describe('ParseError', () => {
  it('is an instance of AnalyzerError', () => {
    expect(new ParseError('syntax error', '/src/foo.ts')).toBeInstanceOf(
      AnalyzerError
    );
  });
...
describe('RuleExecutionError', () => {
  it('is an instance of AnalyzerError', () => {
    expect(
      new RuleExecutionError('my-rule', '/foo.ts', new Error('crash'))
    ).toBeInstanceOf(AnalyzerError);
  });
  ...
  it('includes the cause message', () => {
    const e = new RuleExecutionError(
      'my-rule',
      '/foo.ts',
  ...
  it('has code RULE_EXECUTION_ERROR', () => {
    expect(new RuleExecutionError('r', '/f', new Error()).code).toBe(
...
describe('InfrastructureErrorCollector', () => {
  function makeError(
    recoverable: boolean,
    phase: 'config' | 'planner' | 'engine' = 'engine'
  ) {
    return createInfrastructureError('IOError', {
      cause: 'err',
      recoverable,
      phase,
    });
