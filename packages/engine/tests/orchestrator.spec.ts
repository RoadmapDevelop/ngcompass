/**
 * Unit Tests — orchestrator.ts  (runAnalysis)
 *
 * Tests the high-level analysis orchestration using:
 *   - Precomputed (fully-cached) analysis fast-path
 *   - Local execution with batched tasks (below worker threshold)
 *   - Skipped task result retrieval
 *   - Schema validation of stale precomputed results
 */
...
// ---------------------------------------------------------------------------
// Mock RuleContextFactory so local execution runs without real AST parsing
// ---------------------------------------------------------------------------
...
function makeTask(overrides: Partial<Task> = {}): Task {
    return {
        ...
        options: {},
        inputs: {
            typescript: { path: '/fake/app.component.ts', hash: 'abc', needsAst: true },
        },
...
function makeEmptyPlan(overrides: Partial<ExecutionPlanOutput> = {}): ExecutionPlanOutput {
    return {
        tasks: [],
        skippedTasks: [],
...
function makeValidAnalysisResult(overrides: Partial<AnalysisResult> = {}): AnalysisResult {
    return {
        results: [],
        parseErrors: [],
        stats: {
...
// ---------------------------------------------------------------------------
// Precomputed analysis fast-path
// ---------------------------------------------------------------------------
...
describe('runAnalysis — precomputed analysis', () => {
    ...
    it('ignores precomputed analysis when it fails schema validation', async () => {
        // Invalid schema: missing required stats fields
        const invalidPrecomputed = { results: [], parseErrors: [] } as unknown as AnalysisResult;
        ...
        // Falls through to real execution — result is still Ok
        expect(result.ok).toBe(true);
    });
...
describe('runAnalysis — local execution', () => {
    ...
    it('merges executed results with skipped task cached results', async () => {
        const skippedTask = makeTask({ ruleName: 'cached-rule', taskId: 'skipped-1' });
        ...
        expect(result.ok).toBe(true);
        if (!result.ok) return;
        expect(result.data.results.some((r) => r.ruleName === 'cached-rule')).toBe(true);
    });
    ...
    it('caches the final result when cache and globalHash are provided', async () => {
        ...
        expect(mockCache.analysis.set).toHaveBeenCalledWith('global-hash-abc', expect.any(Object));
    });
});
...
describe('runAnalysis — error handling', () => {
    it('returns Err when an unexpected exception occurs in the orchestration', async () => {
        // Force an error by providing an invalid plan that will throw
