/**
 * Unit Tests — visitor-registry.ts
 *
 * Verifies STREAM_TO_NODE_TYPE mapping and buildVisitorMap dispatch logic.
 */

import { describe, it, expect, vi } from 'vitest';
import { buildVisitorMap, STREAM_TO_NODE_TYPE } from '../src/visitor-registry.js';
...
/** Creates a minimal RuleHandler stub for testing. */
...
/** Creates a simple identity filter: passes every node through as-is. */
const identityFilter = (node: any) => node;

/** A filter that always returns null (nothing passes). */
...
describe('buildVisitorMap', () => {
    ...
    it('stores a bound handle function on the entry', () => {
        ...
        const entry = map.get('CallExpression')![0];
        // Calling entry.handle should delegate to the underlying mock
    ...
    it('handles AngularClass and AnyAngularClass sharing ClassDeclaration key', () => {
        ...
        // Both map to 'ClassDeclaration' → should be grouped together
    ...
    it('returns a ReadonlyMap (its entries are immutable from the outside)', () => {
        ...
        const map = buildVisitorMap(handlers, { CallExpression: identityFilter });
        // The type is ReadonlyMap — attempting to call .set at runtime is fine
        // because the internal Map is cast, but we verify the entries cannot
        // be accidentally mutated by getting a reference and modifying it
        const entries = map.get('CallExpression')!;
        const originalLength = entries.length;
        // Pushing to the returned array (if it were mutable, this would expand it)
        // The cast to ReadonlyArray prevents this at the type level.
