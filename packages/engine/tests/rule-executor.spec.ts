/**
 * Unit Tests — rule-executor.ts
 *
 * Verifies the DI singleton: configureRuleExecutor, getConfiguredExecutor,
 * and getConfiguredChecker.
 */
...
} from '../src/rule-executor.js';
import type { BatchRuleExecutorFn, RuleCheckerFn } from '../src/rule-executor.js';

// Reset the singleton after every test so tests are independent
afterEach(() => {
    // Re-configure with defaults so the next test starts clean.
    // We do this by calling configureRuleExecutor with a checker that returns
    // false (matching the real default) and an executor that throws (matching real default).
    configureRuleExecutor(
...
describe('rule-executor — default state', () => {
    // Note: the module is a singleton; we rely on our afterEach reset above.
    // After afterEach runs it puts things back to the "throw" default.
    ...
    it('default executor throws when called', () => {
        ...
        expect(() => executor([], { filePath: '/a.ts', fileContent: '', locator: null as any }))
...
describe('configureRuleExecutor', () => {
    ...
    it('configured executor is actually invoked', () => {
        ...
        getConfiguredExecutor()([], { filePath: '/a.ts', fileContent: '', locator: null as any });
        expect(invoked).toBe(true);
    });

    it('configured checker is actually invoked', () => {
        const seen: string[] = [];
        const mockChecker: RuleCheckerFn = (name) => { seen.push(name); return true; };
