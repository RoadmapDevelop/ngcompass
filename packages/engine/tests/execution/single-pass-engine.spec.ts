...
import { runSinglePassAnalysis } from '../../src/execution/single-pass-engine.js';
import {
  createCallExpressionRule,
  createTemplateExpressionRule,
} from '../../src/execution/rule-handler.js';
...
describe('runSinglePassAnalysis — no program', () => {
  it('returns one empty RuleResult per rule', () => {
    ...
    const { results } = runSinglePassAnalysis(
      [handler],
      makeContextNoProgram()
    );
...
describe('runSinglePassAnalysis - external template context', () => {
  it('uses the HTML file context for template rules without changing TypeScript rule context', () => {
    ...
    const templateHandler = createTemplateExpressionRule(
      'template-context',
      (_node, context) =>
        makeFailure({
          ruleName: 'template-context',
          filePath: context.filePath,
        })
    );

    const { results } = runSinglePassAnalysis(
      [tsHandler, templateHandler],
      ctx
    );

    expect(
      results.find((result) => result.ruleName === 'ts-context')?.failures[0]
        .filePath
    ).toBe(tsPath);
    expect(
      results.find((result) => result.ruleName === 'template-context')
        ?.failures[0].filePath
    ).toBe(htmlPath);
  });
});
...
describe('runSinglePassAnalysis — error isolation', () => {
  ...
  it('continues to execute other rules after one throws', () => {
    const source = 'foo();';
    const ctx = makeContext(source);
    const badHandler = createCallExpressionRule('bad-rule', () => {
      throw new Error();
    });
    const goodFailure = makeFailure({ message: 'good' });
    const goodHandler = createCallExpressionRule(
      'good-rule',
      () => goodFailure
    );
...
describe('runSinglePassAnalysis — PerformanceReport', () => {
  ...
  it('returns nodesVisited >= 0', () => {
    const { performance } = runSinglePassAnalysis(
      [],
      makeContext('const x = 1;')
    );
  ...
  it('returns one ruleTimings entry per rule', () => {
    ...
    expect(performance.ruleTimings.map((t) => t.ruleName).sort()).toEqual([
      'r1',
      'r2',
    ]);
  });
