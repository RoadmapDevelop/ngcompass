/**
 * Unit Tests — analysis-context.ts
 *
 * Tests createAnalysisContext memoization behaviour, style routing,
 * and error handling — using real temp files via Node's fs module.
 */
...
describe('createAnalysisContext', () => {
    ...
    describe('readFile', () => {
        ...
        it('throws for a non-existent file', async () => {
            const ctx = createAnalysisContext(TEST_ROOT);
            await expect(ctx.readFile(path.join(TEST_ROOT, 'missing.ts')))
                .rejects
        ...
        it('returns the same Promise instance on consecutive calls (memoization)', () => {
            ...
            // Strict equality: same Promise reference
            expect(p1).toBe(p2);
        });
    });
    ...
    describe('getProgram', () => {
        it('returns a parsed Program for a TypeScript file', async () => {
            ...
            // Oxc Program always has a body array
    ...
    describe('getStyle', () => {
        ...
        it('returns a StyleAst-like object for a .css file', async () => {
            ...
            // parseCss returns a CssResult which satisfies StyleAst
    ...
    describe('getTemplate', () => {
        ...
        it('returns undefined for a .ts file with no inline template', async () => {
            // SIMPLE_TS_CONTENT has no @Component decorator, so no inline template
            const ctx = createAnalysisContext(TEST_ROOT);
