/**
 * @fileoverview
 * Catalogue of issue templates emitted by the configuration health checks.
 *
 * Every entry returns a `ConfigIssue` body without the `path` field — checks
 * spread the template and attach their own `path` so issue codes, messages,
 * and suggested fixes live in a single place. Zero-arg templates are stored
 * as plain objects; argument-taking templates are functions.
 */

import type { ConfigIssue } from '@ngcompass/common';

/** Issue template without a `path` — checks supply the path. */
