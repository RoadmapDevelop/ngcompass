/**
 * Unit / Integration Tests — git.ts
 *
 * Uses real temporary git repositories to avoid brittle child_process mocks.
 * Tests cover: isGitRepo, executeGitDiscovery, getRepoFingerprint,
...
// ---------------------------------------------------------------------------
// Shared fixture: a real tiny git repo
// ---------------------------------------------------------------------------
...
beforeAll(async () => {
    // Create a real git repository with two committed files
    gitFixtureDir = await mkdtemp(join(tmpdir(), 'ngcompass-git-'));
    execSync('git init', { cwd: gitFixtureDir, stdio: 'pipe' });
    execSync('git config user.email "ci@ngcompass.test"', { cwd: gitFixtureDir, stdio: 'pipe' });
    ...
    it('resolves files relative to rootDir (absolute paths)', async () => {
        ...
        for (const file of files) {
            ...
            expect(file.startsWith(gitFixtureDir.replace(/\\/g, '/'))).toBe(
                !file.includes('\\') // skip check on Windows with mixed slashes
            );
        }
    });
});
...
describe('getRepoFingerprint', () => {
    ...
    it('returns a stable fingerprint for the same commit', async () => {
        ...
        // May differ slightly if index mtime changes but commit hash should be stable
...
describe('getDirectoryFingerprint', () => {
    it('returns a string matching the dir-N-N-N-N pattern', async () => {
        ...
        // mtimeMs can include a decimal fraction on some platforms
